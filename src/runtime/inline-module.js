...
function replaceImport(code, map) {
  const importExp = /^(\s*import\s+[\s\S]*?from\s*['"`])([\s\S]*?)(['"`])/gim;
...
function getBlobURL(module, replaceImportURL = false, map = {}) {
  ...
  if (loaders) {
    ...
    jsCode = loaders.reduce((code, loader) => {
      ...
      const { code: resolved, map: sourceMap } = transform(code, {
        sourceMap: true,
  ...
  if (moduleID in imports) blobURL = imports[moduleID];
    ...
    if (/^#/.test(moduleID)) {
      module = document.querySelector(
        `script[type="inline-module"]${moduleID}`
      );
    }
    if (!module) {
      module = document.querySelector(
        `script[type="inline-module"][name="${moduleID}"]`
      );
    }
    if (module) {
